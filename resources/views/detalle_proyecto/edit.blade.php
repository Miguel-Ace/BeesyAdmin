...
<form action="{{url('detalle_proyectos/'.$datos->id.'/'.$obtenerId)}}" class="row" method="post">
            ...
            <label for="id_usuario" class="form-label">Usuario</label>
              <select class="form-select" name="id_usuario">
                @foreach ($usuarios as $usuario)
                    @if ($usuario->name === $datos->id_usuario)
                        <option value="{{$usuario->name}}" selected>{{$usuario->name}}</option>
                    @else
    ...
    <a href="{{url('/detalle_proyectos?buscar='.$obtenerId)}}" class="boton regresar"><ion-icon name="arrow-back-outline"></ion-icon>Regresar</a>
