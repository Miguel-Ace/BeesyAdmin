...
@section('creacion')
<form action="{{url('plantilla_detalle_proyectos/'.$datos->id.'/'.$obtenerId)}}" class="row" method="post">
    @csrf
    {{method_field('PATCH')}}
    <div class="row">
        {{-- <div class="col-md-3">
            ...
            <label for="id_usuario" class="form-label">Estado</label>
              <select class="form-select" name="id_estado">
                @foreach ($estados as $estado)
                    @if ($estado->estado === $datos->id_estado)
            ...
            <label for="id_usuario" class="form-label">Etapa</label>
              <select class="form-select" name="id_etapa">
                @foreach ($etapas as $etapa)
                    @if ($etapa->etapa === $datos->id_etapa)
                        <option value="{{$etapa->etapa}}" selected>{{$etapa->etapa}}</option>
                    @else
                        <option value="{{$etapa->etapa}}">{{$etapa->etapa}}</option>
                    @endif
                @endforeach
              </select>
          </div>
    ...
    {{-- <a href="{{url('/plantilla_detalle_proyectos'.'/'.$datos->id.'/'.$obtenerId)}}" class="boton regresar"><ion-icon name="arrow-back-outline"></ion-icon>Regresar</a> --}}
    <a href="{{url('/plantilla_detalle_proyectos?buscar='.$obtenerId)}}" class="boton regresar"><ion-icon name="arrow-back-outline"></ion-icon>Regresar</a>
